...
class LinzNetzClient:
    ...
    async def async_login(self) -> None:
        ...
        form = next((f for f in soup.find_all("form") if f.find("input", attrs={"type": "password"}) is not None), None)
        if form is None:
        ...
        for tag in form.find_all("input"):
            ...
            elif kind in {"text", "email"} and any(x in name.lower() for x in ("user", "mail", "login")):
        ...
        try:
            result = await self._session.post(urljoin(str(response.url), action), data=payload, allow_redirects=True)
    ...
    async def async_fetch_quarter_readings(self, day: date) -> list[QuarterReading]:
        ...
        view_state = form.find("input", attrs={"name": _VIEW_STATE_RE}) or soup.find("input", attrs={"name": _VIEW_STATE_RE})
        ...
        if kwh is None:
            stage = await self._async_apply_choice(form_id, quarter, view_state_name, view_state_value)
            ...
            if kwh is None:
                ...
                raise LinzNetzParseError("Auswahl 'kWh' nach Viertelstunden-Auswahl nicht gefunden")
        ...
        period = form.find("input", attrs={"name": re.compile(r"periodRange$", re.IGNORECASE)})
        ...
        result = await self._session.post(PORTAL_URL, data=payload, headers=headers, allow_redirects=True)
        result.raise_for_status()
        body = await result.text()
        first_page = self._parse_readings(body)
        if not first_page:
            raise LinzNetzParseError(f"Keine Viertelstundenwerte für {day_text} gefunden")

        pagination = self._parse_pagination(body)
        if pagination is None or pagination.row_count <= len(first_page):
            return self._deduplicate_readings(first_page)
        ...
        for first in range(pagination.rows, pagination.row_count, pagination.rows):
            page_payload = self._build_pagination_payload(payload, pagination, first, view_state_name, latest_view_state)
            page_result = await self._session.post(PORTAL_URL, data=page_payload, headers=headers, allow_redirects=True)
            ...
            if not page_readings:
                self._log_pagination_warning(pagination, expected_pages, loaded_pages, len(by_time))
                raise LinzNetzParseError(f"PrimeFaces-Seite ab Datensatz {first} lieferte keine Werte")
            loaded_pages += 1
            for reading in page_readings:
                by_time.setdefault(reading.start_local, reading)
            latest_view_state = self._extract_partial_view_state(page_body) or latest_view_state

        parsed = len(by_time)
        _LOGGER.info("LINZ NETZ Pagination: rows=%s rowCount=%s pages=%s loaded=%s parsed=%s", pagination.rows, pagination.row_count, expected_pages, loaded_pages, parsed)
        if parsed != pagination.row_count or loaded_pages != expected_pages:
            self._log_pagination_warning(pagination, expected_pages, loaded_pages, parsed)
            raise LinzNetzParseError(f"PrimeFaces-Paginierung unvollständig: {parsed} von {pagination.row_count} Werten gelesen")
        return [by_time[key] for key in sorted(by_time)]

    @staticmethod
    def _build_pagination_payload(base: dict[str, str], pagination: PaginationInfo, first: int, view_state_name: str, view_state_value: str) -> dict[str, str]:
        payload = {k: v for k, v in base.items() if k not in {"jakarta.faces.source", "jakarta.faces.partial.execute", "jakarta.faces.partial.render", "jakarta.faces.behavior.event", "jakarta.faces.partial.event"}}
    ...
    @staticmethod
    def _log_pagination_warning(p: PaginationInfo, pages: int, loaded: int, parsed: int) -> None:
        _LOGGER.warning("LINZ NETZ Pagination unvollständig: rows=%s rowCount=%s pages=%s loaded=%s parsed=%s", p.rows, p.row_count, pages, loaded, parsed)

    async def _async_apply_choice(self, form_id: str, choice: ChoiceField, view_state_name: str, view_state_value: str) -> str:
        if not _SELECTED_CLASS_RE.search(choice.name):
            raise LinzNetzParseError("Dynamisches selectedClass-Feld für Viertelstundenwerte fehlt")
        ...
        result = await self._session.post(PORTAL_URL, data=payload, headers={"Faces-Request": "partial/ajax", "X-Requested-With": "XMLHttpRequest"}, allow_redirects=True)
        result.raise_for_status()
    ...
    def _contains_password_form(html: str) -> bool:
        return BeautifulSoup(html, "html.parser").find("input", attrs={"type": "password"}) is not None
    ...
    def _find_consumption_form(cls, soup: BeautifulSoup) -> Tag | None:
        ...
        for form in soup.find_all("form"):
            ...
            score += 3 if cls._contains_marker(form, "ConsumQuarter", _QUARTER_TEXT_RE) else 0
            score += 3 if cls._contains_marker(form, "KWH", _KWH_TEXT_RE) else 0
            score += 2 if form.find("input", attrs={"name": _SELECTED_CLASS_RE}) is not None else 0
    ...
    def _find_named_control(form: Tag, pattern: re.Pattern[str]) -> Tag | None:
        for tag in form.find_all(["input", "select", "textarea"]):
            if pattern.search(str(tag.get("name") or "")) or pattern.search(str(tag.get("id") or "")):
                return tag
        return None

    @classmethod
    def _find_choice_field(cls, form: Tag, desired: str, label_re: re.Pattern[str]) -> ChoiceField | None:
        selected = [tag for tag in form.find_all(["input", "select"]) if _SELECTED_CLASS_RE.search(str(tag.get("name") or ""))]
        for tag in selected:
            if str(tag.get("value") or "").lower() == desired.lower() and tag.get("name"):
                return ChoiceField(str(tag.get("name")), desired)
        if desired.startswith("Consum"):
            for tag in selected:
                if str(tag.get("value") or "").lower().startswith("consum") and tag.get("name"):
                    return ChoiceField(str(tag.get("name")), desired)
        for tag in selected:
            parent = tag.parent
            for _ in range(5):
        ...
        for option in form.find_all("option"):
            ...
            if value.lower() == desired.lower() or label_re.search(option.get_text(" ", strip=True)):
    ...
    @staticmethod
    def _contains_marker(form: Tag, desired: str, label_re: re.Pattern[str]) -> bool:
        low = desired.lower()
        for tag in form.find_all(["input", "option", "select", "label", "button"]):
            if low == str(tag.get("value") or "").lower() or low in str(tag.get("name") or "").lower() or low in str(tag.get("id") or "").lower() or label_re.search(tag.get_text(" ", strip=True)):
                return True
        return False
    ...
    def _find_display_button(form: Tag) -> Tag | None:
        for tag in form.find_all(["button", "input", "a"]):
            if "ANZEIGEN" in (tag.get_text(" ", strip=True) + " " + str(tag.get("value", ""))).upper():
                return tag
    ...
    def _find_render_target(form: Tag, form_id: str) -> str:
        ...
        return str(candidate.get("id")) if candidate is not None and candidate.get("id") else f"{form_id}:list"

    @staticmethod
    def _extract_partial_view_state(body: str) -> str | None:
        match = re.search(r'<update\s+id=["\'](?:jakarta|javax)\.faces\.ViewState[^"\']*["\']\s*>\s*<!\[CDATA\[(.*?)\]\]>', body, flags=re.DOTALL)
    ...
    def _parse_pagination(cls, body: str) -> PaginationInfo | None:
        ...
        if not table_id:
            match = re.search(r'id=["\']([^"\']+:consumptionsTable)["\']', body, flags=re.IGNORECASE)
            table_id = match.group(1) if match else ""
        if not table_id:
            match = re.search(r'["\']([^"\']+:consumptionsTable)["\']', body, flags=re.IGNORECASE)
            table_id = match.group(1) if match else ""
        ...
        return PaginationInfo(table_id, rows_n, count_n) if rows_n > 0 and count_n > 0 else None

    @staticmethod
    def _deduplicate_readings(readings: list[QuarterReading]) -> list[QuarterReading]:
    ...
    def _log_safe_form_diagnostics(cls, soup: BeautifulSoup, form: Tag | None) -> None:
        ...
        values: list[str] = []
        if form is not None:
            relevant = re.compile(r"calendar|consum|quarter|kwh|energy|period|btn|list|selectedClass", re.IGNORECASE)
            ...
            for tag in form.find_all("option") + form.find_all("input", attrs={"type": "radio"}):
                value = str(tag.get("value") or "").strip()
                if value and len(value) <= 64 and value not in values:
                    values.append(value)
        _LOGGER.warning("LINZ NETZ Parser-Diagnose: forms=%s selected_form=%s relevant_controls=%s choice_values=%s", len(soup.find_all("form")), str(form.get("id") or form.get("name") or "") if form is not None else None, names[:30], values[:30])
    ...
    def _parse_readings(body: str) -> list[QuarterReading]:
        ...
        for row in soup.find_all("tr"):
            ...
            if timestamp and value:
                readings.append(QuarterReading(datetime.strptime(timestamp.group(1), "%d.%m.%Y %H:%M"), float(value.group(0).replace(",", "."))))
        return readings
