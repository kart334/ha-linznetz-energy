...
from .validation import RequestedDayMismatch, validate_requested_day
...
class LinzNetzClient:
    ...
    async def async_login(self) -> None:
        ...
        for tag in form.find_all("input"):
            ...
            if kind == "password":
                ...
                item in name.lower() for item in ("user", "mail", "login")
            ):
    ...
    async def async_fetch_quarter_readings(self, day: date) -> list[QuarterReading]:
        ...
        date_from_name = str(date_from.get("name") or "")
        date_to_name = str(date_to.get("name") or "")
        if not date_from_name or not date_to_name:
            raise LinzNetzParseError("Datumsfelder haben keinen übertragbaren Namen")
        ...
        # Browser requests serialize the JSF form. Preserve the current form state,
        # then override only the fields this integration intentionally controls.
        # Processing @form makes the directly posted calendar values part of the
        # JSF lifecycle even when the browser UI would normally update them first.
        ...
        payload.update(
            {
                ...
                date_from_name: day_text,
                date_to_name: day_text,
                kwh.name: kwh.value,
        ...
        result = await self._session.post(
            PORTAL_URL, data=payload, headers=headers, allow_redirects=True
        )
        result.raise_for_status()
        ...
        if pagination is None or pagination.row_count <= len(first_page):
            return self._validated_readings(day, self._deduplicate_readings(first_page))

        latest_view_state = self._extract_partial_view_state(body) or view_state_value
        by_time = {reading.start_local: reading for reading in first_page}
        ...
        if parsed != pagination.row_count or loaded_pages != expected_pages:
            ...
            raise LinzNetzParseError(
                f"PrimeFaces-Paginierung unvollständig: {parsed} von "
                f"{pagination.row_count} Werten gelesen"
            )
        readings = [by_time[key] for key in sorted(by_time)]
        return self._validated_readings(day, readings)

    @staticmethod
    def _validated_readings(
        requested_day: date, readings: list[QuarterReading]
    ) -> list[QuarterReading]:
        """Return readings only when every portal timestamp matches the request."""
        try:
            validate_requested_day(
                requested_day, (reading.start_local for reading in readings)
            )
        except RequestedDayMismatch as err:
            returned = ",".join(day.isoformat() for day in err.returned_days) or "none"
            raise LinzNetzParseError(
                "Portal lieferte Daten für falsches Datum: "
    ...
    def _collect_form_payload(form: Tag) -> dict[str, str]:
        """Serialize non-sensitive successful controls from the consumption form."""
        payload: dict[str, str] = {}
        for tag in form.find_all(["input", "select", "textarea"]):
            ...
            if tag.name == "input":
                ...
                if kind in {"password", "file", "submit", "button", "image", "reset"}:
                    continue
                if kind in {"radio", "checkbox"} and not tag.has_attr("checked"):
                    continue
            ...
            if tag.name == "select":
                ...
                payload[name] = str(selected.get("value") or "") if selected else ""
                continue
    ...
    def _log_pagination_warning(
        pagination: PaginationInfo, pages: int, loaded: int, parsed: int
    ) -> None:
        _LOGGER.warning(
    ...
    def _deduplicate_readings(
        ...
        by_time = {reading.start_local: reading for reading in readings}
    ...
    def _log_safe_form_diagnostics(
        ...
        _LOGGER.warning(
            ...
            str(form.get("id") or form.get("name") or "") if form is not None else None,
            names[:30],
            values[:30],
        )
    ...
    def _parse_readings(body: str) -> list[QuarterReading]:
        ...
        for row in soup.find_all("tr"):
            ...
            if timestamp and value:
                readings.append(
                    QuarterReading(
                        datetime.strptime(timestamp.group(1), "%d.%m.%Y %H:%M"),
